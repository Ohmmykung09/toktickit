  ...
  return <main className="admin-page min-vh-100">
    ...
    {tab === 'queue' ? <StaffWorkspace /> : <section className="admin-layout">
      ...
      <div className="admin-grid"><section className="admin-list" aria-label="Users">
        ...
        {state === 'ready' && users.length > 0 && <><table><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Password</th><th></th></tr></thead><tbody>{users.map((item) => <tr key={item.id}><td>{item.name}</td><td>{item.email}</td><td>{roleLabel(item.role)}</td><td>{item.isActive ? 'Active' : 'Inactive'}</td><td>{item.mustChangePassword ? 'Change required' : 'Current'}</td><td><button aria-label={`Edit ${item.name}`} className="btn btn-sm btn-outline-success" onClick={() => edit(item)} type="button">Edit</button></td></tr>)}</tbody></table><div className="admin-cards">{users.map((item) => <article key={item.id}><header><div><strong>{item.name}</strong><span>{item.email}</span></div><button aria-label={`Edit ${item.name} mobile`} className="btn btn-sm btn-outline-success" onClick={() => edit(item)} type="button">Edit</button></header><dl><div><dt>Role</dt><dd>{roleLabel(item.role)}</dd></div><div><dt>Status</dt><dd>{item.isActive ? 'Active' : 'Inactive'}</dd></div><div><dt>Password</dt><dd>{item.mustChangePassword ? 'Change required' : 'Current'}</dd></div></dl></article>)}</div></>}
      </section>
